    ...
    <div class="font-sans  text-xs w-full max-w-md bg-white rounded-4xl shadow-xl p-8">
        ...
        <form wire:submit="save">
            <div class="mb-5">
                ...
                <input type="text" id="nama_lengkap" wire:model="nama_lengkap" placeholder="Masukka n nama lengkap anda"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500">
                @error('nama_lengkap') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
            </div>

            <div class="mb-5">
                <label for="kategori_instansi" class="block mb-2 text-sm font-medium text-gray-700">Kategori Instansi*</label>
                ...
                @error('kategori_instansi') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
            </div>

            <div class="mb-5">
                <label for="nama_instansi" class="block mb-2 text-sm font-medium text-gray-700">Nama Instansi*</label>
                <input type="text" id="nama_instansi" wire:model="nama_instansi" placeholder="Contoh: BPS Kota Binjai"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500">
                @error('nama_instansi') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
            </div>
            ...
            <div class="mb-6">
                <label for="jenis_pelayanan" class="block mb-2 text-sm font-medium text-gray-700">Jenis Pelayanan*</label>
                <select id="jenis_pelayanan" wire:model="jenis_pelayanan"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500">
                    <option value="" disabled>--Pilih--</option>
                    @foreach($jenisPelayananOptions as $key => $value)
                        <option value="{{ $key }}">{{ $value }}</option>
                    @endforeach
                </select>
                @error('jenis_pelayanan') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
            </div>
